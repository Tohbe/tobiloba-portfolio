/* ════════════════════════════════════════════════════
   CONTENT LOADER — fetches data/content.json and
...
function renderHero(h) {
  ...
  const photo = h.photo
    ? `<img src="${h.photo}" alt="${h.name}" class="avatar-photo" />`
...
function renderSkills(s) {
  const bars = s.competencies.map(c => `
    <div class="bar-item reveal">
      <div class="bar-label">
        <span>${c.name}</span><span>${c.level}%</span>
      </div>
...
function renderProjects(projects) {
  const icons = {
    0: `<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>`,
    1: `<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>`,
    2: `<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>`,
  };
  const iconColors = ['icon-hr', 'icon-sales', 'icon-store'];
  ...
  const cards = projects.map((p, i) => {
    ...
    const imgBlock = p.image
      ? `<div class="proj-img"><img src="${p.image}" alt="${p.title}" /></div>`
      : '';
    ...
    const featuredBadge = p.featured ? '<div class="proj-featured-badge">Featured</div>' : '';
    ...
    return `
      <div class="proj-card ${p.featured ? 'featured' : ''} reveal">
        ...
        ${imgBlock}
        <div class="proj-top">
          <div class="proj-icon ${iconColors[i] || 'icon-store'}">${icons[i] || icons[2]}</div>
    ...
    return `
      <div class="tl-item reveal">
        ...
        <div class="tl-body">
          <div class="tl-header">
            <div>
              <h3 class="tl-role">${e.degree}</h3>
              <p class="tl-org">${e.school}</p>
            </div>
            <div class="tl-meta">
              <span class="tl-date">${e.period}</span>
              <span class="tl-loc">${e.location}</span>
...
function renderCertifications(certs) {
  const cards = certs.map(c => `
    <div class="cert-card reveal">
      <div class="cert-ico ${c.type}">${c.type === 'g' ? 'G' : c.type === 'xl' ? 'XL' : c.type === 'mk' ? 'M' : 'AI'}</div>
      <div class="cert-info">
        <h4>${c.name}</h4>
        <p>${c.issuer}</p>
      </div>
    ...
    <div class="contact-grid">
      <div class="contact-left">
        <div class="cinfo-item">
          ...
          <div>
            <p class="cinfo-label">Email</p>
            <a href="mailto:${c.email}" class="cinfo-val">${c.email}</a>
          </div>
        </div>
        <div class="cinfo-item">
          <div class="cinfo-icon">
          ...
          <div>
            <p class="cinfo-label">WhatsApp</p>
            <a href="https://wa.me/${c.whatsapp.replace(/\D/g,'')}" class="cinfo-val">${c.whatsapp}</a>
          </div>
        </div>
        <div class="cinfo-item">
          <div class="cinfo-icon">
          ...
          <div>
            <p class="cinfo-label">Location</p>
            <span class="cinfo-val">${c.location}</span>
          </div>
        </div>
        <div class="social-row">
          ${c.linkedin ? `<a href="${c.linkedin}" target="_blank" rel="noopener" class="soc-link" aria-label="LinkedIn"><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/></svg></a>` : ''}
      ...
      <form class="contact-form" id="contactForm">
        <div class="form-row">
          <div class="form-group"><input type="text"  placeholder="Your Name"  required class="finput" /></div>
          <div class="form-group"><input type="email" placeholder="Your Email" required class="finput" /></div>
        </div>
        <div class="form-group"><input type="text" placeholder="Subject" class="finput" /></div>
        <div class="form-group"><textarea placeholder="Your Message" rows="5" required class="finput ftextarea"></textarea></div>
...
function initTypewriter(rolesData) {
  ...
  // rolesData may be array of strings OR array of {title:...} objects
